...
namespace PM3D
{
	...
	public:
		...
		int GetNombreSubmesh() const override { return NombreSubmesh; }
		...
		void CopieSubmeshIndex(std::vector<int>& dest) const override { dest = SubmeshIndex; }
		...
		std::vector<int> GetIndex() const override { return tabIndex; };
	private:
		// FONCTIONS
		...
		std::vector<int> tabIndex;					// Index pour les polygones (triangles)
													...
													// *** Les sous-objets
		std::vector<int> SubmeshIndex;			// index
		int NombreSubmesh;					// nombre de sous objets
