...
namespace PM3D
{
	...
	bool CChargeurOBJ::LireFichier(const std::string& nomFichier)
	{
		...
		SubmeshIndex.push_back(static_cast<int>(tabIndex.size()));
	...
	void CChargeurOBJ::TraiterLigneOBJ(const std::string& Instr)
	{
		...
		case 'o':	// g - definir un groupe
			...
			SubmeshIndex.push_back(static_cast<int>(tabIndex.size()));  // Point de départ des indices pour ce subset
			NombreSubmesh++;
		...
		case 'u':	//usemtl - which material to use
		{
			...
			{
				iss >> str;
				materialName[NombreSubmesh - 1] = str;
			};
		}
