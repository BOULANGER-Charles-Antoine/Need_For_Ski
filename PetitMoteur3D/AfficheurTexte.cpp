...
namespace PM3D
{
	...
	CAfficheurTexte::CAfficheurTexte(int largeur, int hauteur, Gdiplus::Font* pPolice)
		: pDispo(CMoteurWindows::GetInstance().GetDispositif())
		, TexWidth(largeur)
		, TexHeight(hauteur)
		, pFont(pPolice)
		, pTexture(nullptr)
		...
		, pBlackBrush(nullptr)
	{
		...
		auto const r = Gdiplus::Rect(0, 0, TexWidth, TexHeight);
		pCharBitmap->LockBits(&r, Gdiplus::ImageLockModeRead, PixelFormat32bppARGB, &bmData);
	...
	void CAfficheurTexte::Ecrire(const std::wstring& s)
	{
		...
		// Transférer
		Gdiplus::BitmapData bmData;
		auto r = Gdiplus::Rect(0, 0, TexWidth, TexHeight);
		pCharBitmap->LockBits(&r, Gdiplus::ImageLockModeRead, PixelFormat32bppARGB, &bmData);
