...
{
	float PI = atan(1) * 4.0;
	float angleMin = 70 * PI / 180.0;
	float angleMax = 60 * PI / 180.0;
	float angle = acos(abs(dot(float3(0, 1, 0), vs.WorldNorm)) / length(vs.WorldNorm));
	...
	//Alpha blending
	float alpha = (angle - angleMin) / (angleMax - angleMin);
	if (alpha < 0.0)
	{
		alpha = 0.0;
	}
	...
	couleurTexture = textureForeground.Sample(SampleState, vs.coordTex).rgb * (1 - alpha) +
		             textureBackground.Sample(SampleState, vs.coordTex).rgb * alpha;

	// I = A + D * N.L + (R.V)n
	couleur = couleurTexture * vAEcl.rgb * 4 * vAMat.rgb +
